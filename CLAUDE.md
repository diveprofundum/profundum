...
The schema (implemented in Swift GRDB migrations 001–017) models technical diving with CCR support:
...
| 8. Second-opinion review | Launch a review subagent with a non-primary model — use the latest GPT Sol model (currently `gpt-5.6-sol-medium`) when working in Claude, or vice versa | Independent review for bugs, edge cases, improvements. No external MCP required — Cursor's multi-model subagents provide the independent perspective natively. |
